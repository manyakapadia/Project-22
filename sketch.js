...
function draw(){
    ...
    if(yellowBox.x - ball.x < ball.width/2 + yellowBox.width/2 && 
        ball.x - yellowBox.x < ball.width/2 + yellowBox.width/2 &&
        yellowBox.y - ball.y < ball.height/2 + yellowBox.height/2 &&
        ball.y - yellowBox.y < ball.height/2 + yellowBox.height/2){
            boing.play();
        }

    if(greenBox.x - ball.x < ball.width/2 + greenBox.width/2 && 
            ball.x - greenBox.x < ball.width/2 + greenBox.width/2 &&
            greenBox.y - ball.y < ball.height/2 + greenBox.height/2 &&
            ball.y - greenBox.y < ball.height/2 + greenBox.height/2){
                boing.play();
        }
    
    if(redBox.x - ball.x < ball.width/2 + redBox.width/2 && 
            ball.x - redBox.x < ball.width/2 + redBox.width/2 &&
            redBox.y - ball.y < ball.height/2 + redBox.height/2 &&
            ball.y - redBox.y < ball.height/2 + redBox.height/2){
                boing.play();
     }
}
